...
public class StateManager extends StateProcessor {
    ...
    public void init(){
        ...
        if (stateString != null) {
            try {
                ...
                stateMachine.start(new ActivityInfo(AppStates.Login, LoginActivity.uiActivity));
            }
            //storeState();
        } else {
            Log.e(TAG, "Setting stack to empty.");
            stateMachine.start(new ActivityInfo(AppStates.Hello, HelloActivity.uiActivity));
        }
    ...
    public enum AppEvents {
        ...
        SignUpSuccessful,
        SignUpUserInAceds,
        ViewMyAccount, StartApplication, ResumeApplication, Yes, No, ErrorHappened
    }
    ...
    public void configStates() {
        ...
        stateMachine.from(AppStates.ChooseFinancialAssistance).on(AppEvents.Yes).to(AppStates.AcctCreate, new LaunchActivity(AcctCreate.uiActivity));
        ...
        stateMachine.from(AppStates.Login)
            ...
            @Override
            public void call(StateMachine stateMachine, StateManager stateManager, AppEvents appEvents, AppStates leavingState, AppStates enterState) throws IOException, CoverageException {
                ...
                stateMachine.push(new ActivityInfo(enterState, PlanShoppingChoicesActivity.uiActivity));
            }
        });
    ...
    private void initBasicPlanShoppingStates(StateMachine stateMachine){
        ...
        stateMachine.from(AppStates.PlanSelector).on(AppEvents.Details).to(AppStates.PlanShoppingDetails, new LaunchActivity(PlanDetailsActivity.uiActivity));
    ...
    public static class GetQuestions implements StateMachineAction {
        ...
        @Override
        public void call(StateMachine stateMachine, StateManager stateManager, AppEvents event, AppStates leavingState, AppStates enterState) throws IOException, CoverageException {
            stateMachine.push(new WaitActivityInfo<AppEvents, AppStates>(AppStates.GetQuestions, null));
    ...
    public static class VerifyUser implements StateMachineAction {
        @Override
        public void call(StateMachine stateMachine, StateManager stateManager, AppEvents event, AppStates leavingState, AppStates enterState) throws IOException, CoverageException {
            stateMachine.push(new WaitActivityInfo(enterState, null));
            stateManager.showWait();
    ...
    public class DialogInfo extends StateInfoBase {
        ...
        public DialogInfo(AppStates state, StateManager.UiDialog uiDialog, ActivityInfo activityInfo) {
            super(state);
