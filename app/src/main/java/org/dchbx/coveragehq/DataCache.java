...
public class DataCache implements IDataCache {
    ...
    public BrokerAgency getBrokerAgency(DateTime time) {
        ...
        && time.compareTo(brokerAgencyTime.plus(ServiceManager.getServiceManager().getEnrollConfig().getCacheTimeout()))<=0){
            return brokerAgency;
        }
        return null;
    }
    ...
    public Employer getEmployer(String id, DateTime time) {
        if (employers.containsKey(id)){
            ...
            if (time.compareTo(employerCachedData.time.plus(ServiceManager.getServiceManager().getEnrollConfig().getCacheTimeout())) <= 0){
    ...
    public Roster getRoster(String id, DateTime time) {
        if (rosters.containsKey(id)){
            ...
            if (time.compareTo(rosterCachedData.time.plus(ServiceManager.getServiceManager().getEnrollConfig().getCacheTimeout())) <= 0){
    ...
    public List<Service> getServices(String id, DateTime time) {
        if (servicesCache.containsKey(Default_Id)){
            ...
            if (time.compareTo(cachedData.time.plus(ServiceManager.getServiceManager().getEnrollConfig().getCacheTimeout())) <= 0){
