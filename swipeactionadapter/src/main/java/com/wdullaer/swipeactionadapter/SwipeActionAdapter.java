        ...
        SwipeActionTouchListener.ActionCallbacks
{
    ...
    public SwipeActionAdapter(SwipeListArrayAdapter baseAdapter){
        super(baseAdapter);
    }

    @Override
    public View getView(final int position, final View convertView, final ViewGroup parent){
        if (!getAdapter().isItemSwipeable(position)){
            return getAdapter().getView(position, convertView, parent);
        }


        SwipeViewGroup output = null;
        if (convertView != null
            && convertView instanceof SwipeViewGroup){
            output = (SwipeViewGroup)convertView;
        }
